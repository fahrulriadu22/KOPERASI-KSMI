...
import '../services/global_session_checker.dart'; // ✅ IMPORT BARU
import 'dart:async';
...
class _DashboardMainState extends State<DashboardMain> {
  ...
  // ✅ PERBAIKAN: Inisialisasi userData dengan nilai default
  Map<String, dynamic>? userData; // Jadikan nullable
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _checkMultiDeviceImmediately();  // ✅ TAMBAHIN BARIS INI
      _initializeData();
      _startSessionChecker();
    });
  }

  // ✅ TAMBAHIN: CHECK MULTI DEVICE IMMEDIATELY SAAT STARTUP
  void _checkMultiDeviceImmediately() async {
    try {
      final userData = await SessionManager.getUserData();
      final userId = userData?['user_id']?.toString() ?? '';
      
      if (userId.isNotEmpty) {
        final isOtherDevice = await DeviceService.detectOtherDeviceLogin(userId);
        if (isOtherDevice) {
          print('🚨 MULTI-DEVICE DETECTED ON STARTUP - FORCE LOGOUT');
          _performLogout();
        }
      }
    } catch (e) {
  ...
  // ✅ SESSION CHECKER UNTUK DASHBOARD
  void _startSessionChecker() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      globalSessionChecker.startSessionChecker(context);
      print('✅ Global session checker started in Dashboard');
    });
  }

  // HAPUS method didChangeAppLifecycleState

  @override
  void dispose() {
    _autoRefreshTimer?.cancel();
    // HAPUS: WidgetsBinding.instance.removeObserver(this);
    globalSessionChecker.stopSessionChecker();
    super.dispose();
  }
...
Future<void> _initializeData() async {
  try {
    ...
    // ✅ CEK TOKEN DULU SEBELUM LOAD DATA
    final tokenValid = await _checkTokenValidity();
    if (!tokenValid) {
      return; // Stop initialization jika token invalid
    }
  ...
  print('🧪 Simulated token expired - should trigger auto logout');
  
  // Trigger session check
  _checkTokenValidity();
}

  // ✅ PERBAIKAN: LOAD CURRENT USER DENGAN CHECK TOKEN VALIDITY
  Future<void> _loadCurrentUser() async {
    try {
      ...
      final tokenValid = await _checkTokenValidity();
      if (!tokenValid) {
        return; // Stop loading jika token invalid
      }
      
      // ✅ LANJUT LOAD USER DATA
      ...
      if (profileResult['success'] == true && profileResult['data'] != null) {
        ...
        // ✅ CEK APAKAH KARENA TOKEN EXPIRED
        if (profileResult['token_expired'] == true) {
          print('🚨 Token expired detected in API response');
          _showTokenExpiredDialog();
          return;
        }
        
        print('⚠️ getUserProfile failed, using local data');
    ...
    // ✅ TAMBAHIN: CHECK TOKEN VALIDITY METHOD
  Future<bool> _checkTokenValidity() async {
    try {
      ...
      // ✅ COBA API CALL RINGAN
      final userInfo = await _apiService.getUserInfo();
      
      if (userInfo['token_expired'] == true) {
...
// ✅ METHOD BARU: LOAD USER DARI LOCAL STORAGE SAJA
Future<void> _loadUserFromLocalStorage() async {
  try {
    print('📁 Loading user data from local storage...');
    ...
    if (currentUser != null && currentUser is Map<String, dynamic> && currentUser.isNotEmpty) {
      ...
      print('⚠️ No local user data, using initial data from widget');
      setState(() {
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              'Selamat datang, ${userData?['nama'] ?? 'User'}!', // ✅ GUNAKAN NULL CHECK
              style: TextStyle(
                fontSize: 14,
...
Widget _buildMainScreen() {
  // ✅ PERBAIKAN: PASTIKAN userData TIDAK NULL
  final Map<String, dynamic> safeUserData = userData ?? {'username': 'User', 'nama': 'User'};
  
  final List<Widget> pages = [
    DashboardScreen(user: safeUserData, onRefresh: _refreshUserData),
    RiwayatTabunganScreen(user: safeUserData),
    RiwayatAngsuranScreen(user: safeUserData),
    ProfileScreen(
      user: safeUserData, 
...
Widget _buildNavItem({
  ...
  // ✅ PERBAIKAN: FIX GETTER CURRENT USER
  Map<String, dynamic> get currentUser => userData ?? {'username': 'User', 'nama': 'User'};
