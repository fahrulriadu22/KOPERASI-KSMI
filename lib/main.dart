...
// ✅ PERBAIKAN: SESSION CHECKER YANG LEBIH AMAN
void _startSessionChecker() {
  // Check setiap 60 detik (bukan 30 detik)
  Future.delayed(const Duration(seconds: 60), () {
    if (mounted && _isLoggedIn && _userData.isNotEmpty) {
      _checkSessionValidity();
      _startSessionChecker(); // Loop
    }
  });
}
  ...
  @override
  void dispose() {
    // ✅ SIMPAN WAKTU APP DITUTUP
    DeviceService.saveAppCloseTime();
    super.dispose();
  }

  @override
  void didChangeAppLifecycleState(AppLifecycleState state) {
    print('📱 Main App lifecycle state: $state');
    
    if (state == AppLifecycleState.paused || state == AppLifecycleState.detached) {
      // ✅ PERBAIKI: Gunakan method yang benar
      DeviceService.saveAppCloseTime(); // ← INI SUDAH BENAR
    }
  }
...
// ✅ PERBAIKAN: CHECK SESSION VALIDITY YANG LEBIH SMART
...
🔐 Session Check Result:
  ...
  - Other Device Login: $isOtherDeviceLogin
  - Current User: $userId
''');
    
    if (!isSessionValid || isOtherDeviceLogin) {
      print('🚨 Session invalid or other device login detected');
      _showSessionExpiredDialog();
    }
  } catch (e) {
    print('❌ Error checking session validity: $e');
    // Jangan logout jika error, biarkan user tetap login
  }
}

void _showSessionExpiredDialog() {
  // Cek dulu apakah context masih mounted
  if (!mounted) return;
  ...
  showDialog(
    ...
    builder: (context) => AlertDialog(
      title: const Text('Login dari Device Lain'),
      content: const Text(
        'Akun Anda telah login dari device lain. '
        'Anda akan di-logout secara otomatis untuk keamanan.'
      ),
      actions: [
        TextButton(
          onPressed: () {
    ...
    try {
      ...
      if (isLoggedIn) {
        ...
        if (userData != null && userData.isNotEmpty) {
          // ✅ TAMBAHIN CHECK MULTI-DEVICE DISINI
          ...
          if (userId.isNotEmpty) {
            ...
            if (isOtherDevice) {
              print('🚨 MULTI-DEVICE DETECTED ON APP START - LOGOUT');
              await _handleLogout();
              return;
            }
          }
  ...
  // ✅ PERBAIKAN: HANDLE LOGOUT YANG LEBIH BERSIH
  Future<void> _handleLogout() async {
    try {
      ...
      try {
        ...
        print('⚠️ Error cancelling tasks: $e');
      }
