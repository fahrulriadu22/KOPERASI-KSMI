...
Future<Map<String, String>> getProtectedHeaders() async {
  try {
    ...
    // ✅ RETURN HEADERS BASIC MESKI ERROR
    return {
      'DEVICE-ID': _deviceId,
      'DEVICE-TOKEN': _deviceToken,
      'Content-Type': 'application/x-www-form-urlencoded',
...
Future<Map<String, dynamic>> uploadBuktiTabunganWithDummy({
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      print('🚨 Saldo API 401 - Token expired');
      return {
        'success': false,
        'message': 'Sesi telah berakhir',
        'token_expired': true, // ✅ INI PENTING!
        'status_code': 401
      };
    } else {
      print('❌ Server error: ${response.statusCode}');
    ...
    if (response.statusCode == 200 || response.statusCode == 400) {
      ...
      if (data['status'] == true) {
        ...
        if (userId.isNotEmpty) {
          ...
          await DeviceService.updateLastLoginTime();
          print('✅ Login session saved for user: $userId');
        }
        
        // ✅ SIMPAN CI SESSION JIKA ADA
...
Future<Map<String, dynamic>> logout() async {
 ...
 // ✅ TAMBAHIN METHOD INI BARU - UNTUK HANDLE TOKEN EXPIRED OTOMATIS
  Future<void> _handleTokenExpired(BuildContext? context) async {
    print('🚨 TOKEN EXPIRED DETECTED - LOGGING OUT...');
    
    try {
      // ✅ CLEAR LOCAL DATA
      await _clearToken();
      
      // ✅ TRIGGER GLOBAL SESSION CHECKER LOGOUT
      final globalChecker = GlobalSessionChecker();
      globalChecker.forceLogout();
...
Future<Map<String, dynamic>> getAllInbox() async {
  // ✅ BUAT CUSTOM HTTP CLIENT DENGAN SSL BYPASS (DEV ONLY)
  ...
  try {
    ...
    print('📥 Getting inbox with IOClient...');
    ...
    final response = await client.post(
      ...
      headers: {
        ...
        'Accept': '*/*', // ← PAKAI INI, BUKAN 'application/json'
        'User-Agent': 'Mozilla/5.0 (Linux; Android 10; SM-G973F) AppleWebKit/537.36', // ← PAKAI USER-AGENT REAL
      },
      body: '',
    ).timeout(const Duration(seconds: 30));
    ...
    print('📡 IOClient Response: ${response.statusCode}');
    
    if (response.statusCode == 200) {
      final data = jsonDecode(response.body);
      print('✅ IOClient SUCCESS: $data');
      return processInboxData(data);
    }
    
  } catch (e) {
    print('❌ IOClient Error: $e');
    client.close();
  }
  
  return fallbackInboxData();
}
...
Future<void> debugUserData() async {
  ...
  // TAMBAHKIN METHOD INI DI api_service.dart
Future<bool> isTokenValid() async {
  try {
    ...
    // Token valid jika:
    // 1. Success true DAN
    // 2. Tidak ada token_expired
    return userInfoResult['success'] == true && 
           userInfoResult['token_expired'] != true;
  } catch (e) {
    // Untuk error network, anggap token masih valid (biar user bisa coba login)
    print('⚠️ Token validation error, assuming valid: $e');
    return true;
  }
}

  // DI api_service.dart - COMMENT/HAPUS METHOD INI
/*
